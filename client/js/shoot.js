(() => {
    ...
    localCar.sensor = new Sensor(localCar)
    localCar.brain = new NeuralNetwork(
        [localCar.sensor.rayCount, 6, 5]
    )

    if(localStorage.getItem("bestBrain")){            
        localCar.brain=JSON.parse(localStorage.getItem("bestBrain"))
    }

    $save = document.querySelector('.save');
  
    $save.onclick = () => {
        localStorage.setItem("bestBrain", JSON.stringify(localCar.brain));
    }
    ...
    $mutate = document.querySelector('.mutate');

    $mutate.onclick = () => {
        if(localStorage.getItem("bestBrain"))
        {
            localCar.brain=JSON.parse(localStorage.getItem("bestBrain"));
            NeuralNetwork.mutate(localCar.brain, 0.25);
        }
    }    
    ...
    function updateCar (car, i) {
        ...
        if (car.sensor != undefined) {
            car.sensor.update([], cars, localCar)
        }
    ...
    setInterval(() => {
      let changed;
        ...
        const offset = localCar.sensor.readings.map(
            s => s == null ? 0 : 1 - s.offset
        )

        const output = NeuralNetwork.feedForward(offset, localCar.brain)
